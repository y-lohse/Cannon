	...
	__loaded: function(){
		...
		if (this.includes.length > 0){
			...
			for (var i = 0; i < this.includes.length; i++){
				(function(script, ls){
					Cannon.Logger.log(script);
					ls.add(script);
					Cannon.getScript(script, function(){
						ls.ready(script);
					});
				})(this.includes[i], ls);
			}
	...
	*/
	cloneArray: function(array){
		...
		for (var i = 0, l = array.length; i < l; i++){
			if (array[i] instanceof Array) a[i] = Cannon.Utils.cloneArray(array[i]);
			else a[i] = array[i];
		}
	...
	/**
	Returns the array where every value apperas only once
	@param (array) array the array to clone
	@return a new array
	@type array
	*/
...
Cannon.LoadingSet = Cannon.ClassFactory.extend({
	...
	*/
	ready: function(piece){
		var index = this.__findPiece(piece);
	...
	*/
	...
	/**
	Refreshes the amount of files loaded. You don't need to call this function yourself, it gets called when a piece has finished loading
	*/
	__update: function(){
		this.total = 0;
		this.loaded = 0;
